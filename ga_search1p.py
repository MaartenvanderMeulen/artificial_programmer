...
def ga_search_impl(toolbox):
    ...
    try:
        ...
        toolbox.gen = 0
        toolbox.real_gen = 0
        while toolbox.parachute_level in range(len(toolbox.ngen)):
            while toolbox.gen < toolbox.ngen[toolbox.parachute_level]:
                ...
                max_count = (toolbox.ngen[0] + 5 if meta_evolution_count else 5) - 1 # 4x stuck == 5 dezelfde waarden achter elkaar
                if stuck_count_since_last_meta_evolution >= max_count and len(toolbox.metaevolution_strategies) > 0:
                ...
                if toolbox.f and toolbox.verbose >= 1:
                    toolbox.f.write(f"gen {toolbox.real_gen:2d} best {population[0].eval:7.3f} sc {stuck_count}")
                    toolbox.f.write(f" goc {meta_evolution_count} taboo {str(toolbox.taboo_set)}\n")
                    #count_best = sum([1 for ind in population if ind.eval == population[0].eval])
                    #childN = toolbox.nchildren[toolbox.parachute_level]
                    #print(f"gen {toolbox.gen:3d}, besteval {population[0].eval:7.3f}, len(pop) {len(population):4d}, nchildren {childN:4d}, count_best {count_best:4d}")
                ...
                population.sort(key=lambda item: item.eval)
                if stuck_count == 0:
                    toolbox.leftovers += population[toolbox.pop_size[toolbox.parachute_level]:]
    ...
    best = population[0]
    return best, toolbox.real_gen + 1
